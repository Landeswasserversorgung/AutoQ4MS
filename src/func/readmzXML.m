...
% License           : BSD 3-Clause License (Copyright © 2022 Adrian Haun) see THIRD_PARTY_LICENSES/LICENSE_AriumMS_Adrian_Haun.txt
...
options.MSLevel = 1; % Standardwert

% Verarbeitung optionaler Eingaben
if ~isempty(varargin)
    for i = 1:2:length(varargin)
        if strcmpi(varargin{i}, 'MSLevel')
            options.MSLevel = varargin{i+1};
        end
    end
end
...
if ~ischar(DataPath) && ~isstring(DataPath)
    error('DataPath muss vom Typ char oder string sein.');
end

% Stellen Sie sicher, dass MSLevel eine ganze Zahl ist
if ~isnumeric(options.MSLevel) || options.MSLevel ~= floor(options.MSLevel)
    error('MSLevel muss eine ganze Zahl sein.');
end

% Ihre Funktionslogik hier
[~,~,endian] = computer;
try
